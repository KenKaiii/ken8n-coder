...
export class N8nClient {
  ...
  private validateWorkflow(workflow: N8nWorkflow): N8nWorkflowNode {
    ...
    if (!workflow.nodes || !Array.isArray(workflow.nodes)) {
      throw new Error('Invalid workflow: Missing or invalid "nodes" array. Workflow must have: {"name": "...", "nodes": [...]}');
    }
   ...
   */
  private cleanWorkflowForApi(workflow: N8nWorkflow): Omit<N8nWorkflow, 'id' | 'active' | 'createdAt' | 'updatedAt' | 'versionId' | 'staticData'> {
  ...
  async updateWorkflow(workflowId: string, updates: Partial<N8nWorkflow>): Promise<any> {
    try {
      ...
      if (updates.nodes) {
        const fixedWorkflow = this.ensureWorkflowStructure(updated);
        this.validateWorkflow(fixedWorkflow);
      }

      // Clean and update
      const cleanWorkflow = this.cleanWorkflowForApi(updated);
      const result = await this.makeRequest('PUT', `/workflows/${workflowId}`, cleanWorkflow);
